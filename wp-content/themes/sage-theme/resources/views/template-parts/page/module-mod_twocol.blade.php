        ...
        @for ($i = 0; $i < 10; $i++)
            <div class="lg:flex lg:justify-between sm:block lg:w-full">
                <div class="xl:min-w-275 lg:min-w-370">
                    <h5>{{ $data->module['contentrepatercolfirst'][$i]->title }}</h5>
                    <p>{{ $data->module['contentrepatercolfirst'][$i]->contentd }}</p>
                </div>
                <div class="xl:min-w-275 lg:mt-0 lg:ml-27 md:mt-27 lg:min-w-395 xl:ml-44">
                    <h5>{{ $data->module['contentirepeatercolsecond'][$i]->title }}</h5>
                    <p>{{ $data->module['contentirepeatercolsecond'][$i]->content }}</p>
                </div>
            </div>
        @endfor
    </div>
