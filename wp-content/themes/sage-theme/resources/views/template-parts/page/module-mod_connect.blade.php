<section class="connect lg:flex sm:block sm:mt-21 lg:mt-33 xl:mt-17 2xl:mt-47">
    ...
    <aside class="sm:w-full sm:pt-147 sm:px-5 lg:pt-2 md:px-63 lg:pl-47 lg:pr-51 xl:pl-80 xl:pr-45 2xl:pr-161 static xl:pt-85">
        <h2>{!! $data->module['titleconnect'] !!}</h2>
        <div class="sm:mt-14 lg:mt-8">
          @foreach ($data->module['contentconnect'] as $item)
          <p>{!! $item->rowconnect !!}</p>
          @endforeach
            <a aria-label="Connect" href="{{ $data->module['linkconnect']->url}}" target="{{ $data->module['linkconnect']->target }}" class="btn btn-brown1 sm:hidden xl:inline-block">{{ $data->module['linkconnect']->title }}</a>
            </div>
    </aside>
</section>
<?php
